...
export default async function HomePage() {
  ...
  const topSellers = topSectionProducts.length
    ? topSectionProducts
    : pickConsistentProducts(allProducts, 6);
  const newArrivalsProducts = newSectionProducts.length
    ? newSectionProducts
    : pickConsistentProducts(allProducts, 6);
  const bestDealsProducts = bestSectionProducts.length
    ? bestSectionProducts
    : pickConsistentProducts(allProducts, 6);
          ...
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="space-y-6">
              {transformedTrends.length > 0 ? (
                transformedTrends
                  .reduce((pairs: TrendItem[][], item: TrendItem, index: number) => {
                    if (index % 2 === 0) {
                      pairs.push([item]);
                    } else {
                        ...
                        >
                          ...
                          <div className="absolute top-0 left-0 z-10 p-6">
                            <div className={isWide ? "max-w-xs" : "max-w-50"}>
                              ...
                              <p className="text-slate-200 text-sm mb-3">{trend.subtitle}</p>
                            </div>
                          </div>
                        </WobbleCard>
                      );
