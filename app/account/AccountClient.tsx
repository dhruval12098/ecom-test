...
function AccountPageInner() {
  ...
  useEffect(() => {
    const loadOrders = async () => {
      try {
        const cacheKeyParts = [customerId, user.email || authUser?.email, user.phone || authUser?.phone]
          .filter(Boolean)
          .join("|");
        const cacheKey = cacheKeyParts ? `account:orders:${cacheKeyParts}` : null;
        const cacheTtlMs = 5 * 60 * 1000;
        const now = Date.now();
        let cachedPayload: { data: Order[]; savedAt: number } | null = null;
        if (cacheKey && typeof window !== "undefined") {
          try {
            const raw = window.localStorage.getItem(cacheKey);
            if (raw) {
              const parsed = JSON.parse(raw);
              if (parsed?.data && typeof parsed?.savedAt === "number" && now - parsed.savedAt < cacheTtlMs) {
        ...
        const mapped = data.map((order: any) => ({
          ...
          paymentStatus: order.payment_status ? String(order.payment_status).toLowerCase() : null,
          createdAt: order.created_at || null,
          total: Number(order.total_amount || 0),
          items: Number(order.items_count || 0)
        }));
        const signature = mapped.map((o) => `${o.id}:${o.status}:${o.paymentStatus}:${o.total}:${o.items}:${o.createdAt}`).join("|");
        const existingSignature = orders.map((o) => `${o.id}:${o.status}:${o.paymentStatus}:${o.total}:${o.items}:${o.createdAt}`).join("|");
        ...
        if (cacheKey && typeof window !== "undefined") {
          try {
            window.localStorage.setItem(cacheKey, JSON.stringify({ data: mapped, savedAt: now }));
          } catch {
            // ignore cache errors
          }
        }
                              ...
                              <td className="px-4 py-3 text-right whitespace-nowrap">
                                <div className="flex justify-end items-center gap-3">
                                  {(() => {
                                    ...
                                    const isExpired = Number.isFinite(createdAt) && Date.now() - createdAt >= 24 * 60 * 60 * 1000;
                                      const canRetry = order.status === "Pending" && !isExpired;
