...
class ApiService {
  ...
  static async getActiveSchedule(productId, variantId) {
    try {
      const cacheKey = variantId !== undefined && variantId !== null && variantId !== ''
        ? `active-schedule:${productId}:${variantId}`
        : `active-schedule:${productId}`;
      const cached = ApiService._getCached(cacheKey);
      if (cached) return cached;
      const query = variantId !== undefined && variantId !== null && variantId !== ''
        ? `productId=${productId}&variantId=${variantId}`
        : `productId=${productId}`;
      const response = await fetch(`${API_BASE_URL}/api/scheduled-pricing/active?${query}`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const result = await response.json();
